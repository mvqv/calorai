...
interface AppState {
  ...
  addLogItem: (logItem: Omit<LogItem, 'id' | 'calories' | 'protein' | 'fat' | 'carbs' | 'foodItemId'> & { foodItem: FoodItem }) => void;
...
/** Recalculate totals from items array — pure function, no allocation overhead */
...
export const useAppStore = create<AppState>()(
  persist(
    ...
    }),
    {
      ...
      // Throttle persist writes — don't write to localStorage on every keystroke
      skipHydration: false,
    }
  )
);

/** Stable derived selectors — use these to avoid unnecessary re-renders */
export const selectTodayCalories = (state: AppState) => {
  const today = new Date().toISOString().split('T')[0];
  return state.dailyLogs.find((l) => l.date === today)?.totalCalories ?? 0;
};

export const selectTodayWater = (state: AppState) => {
  const today = new Date().toISOString().split('T')[0];
